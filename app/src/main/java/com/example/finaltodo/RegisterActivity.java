...
public class RegisterActivity extends AppCompatActivity {
    ...
    EditText edUsername, edEmail, edPassword, edReenter;
    Button btn;
    TextView tvl;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edReenter =findViewById(R.id.editTextRegRePassword);
        btn =findViewById(R.id.buttonRegister);
        tvl =findViewById(R.id.textViewRegLogin);
        ...
        btn.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v) {
                ...
                String reenter =edReenter.getText().toString();
                Database db = new Database(getApplicationContext(),"finaltodo",null,1);
                if(username.length()==0 || email.length()==0 || password.length()==0 || reenter.length()==0){
                    Toast.makeText(getApplicationContext(),"Please Fill All Details",Toast.LENGTH_SHORT).show();
                }
                else{
                    if(password.compareTo(reenter)==0){
                        if(isValid(password)){
